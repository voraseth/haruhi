...
static int c2rem = 0;   /* T-n6-c2: number of TYPE-II cycles (2 nc slots, 3 classes, l1+l2=3) still to place */
...
static int eg(void){
    ...
    for(int t = 0; t < 6; t++){
        ...
        if(om[o]) continue;
        if(ncy > c2rem)                       /* TYPE-I cycles remaining */
        ...
        if(c2rem > 0){                        /* TYPE-II: nc at q and q+2 (TC2c: l1+l2=3, l_j>=1) */
            for(int q = 0; q < 5; q++){
                int nc1 = q, nc2 = (q + 2) % 5;
                if(pc == nc1 || pc == nc2) continue;
                int ok = 1;
                for(int j = 0; j < 5; j++) if(j != nc1 && j != nc2 && cget(clsT[operm[o][j]])){ ok = 0; break; }
                if(!ok) continue;
                om[o] = 31; ncy--; c2rem--;
                for(int j = 0; j < 5; j++) if(j != nc1 && j != nc2) cset(clsT[operm[o][j]]);
                if(eg()) return 1;
                for(int j = 0; j < 5; j++) if(j != nc1 && j != nc2) cclr(clsT[operm[o][j]]);
                c2rem++; ncy++; om[o] = 0;
            }
        }
    }
...
int main(int argc, char **argv){
    ...
    if(getenv("C2N")) c2rem = atoi(getenv("C2N"));   /* T-n6-c2 extension */
    ...
    /* E-9: instance consistency.  The piece multiset must cover the 120
       ...
       components, the cycles, the span totals, and the defect piece.
       ...
       THIS ENGINE'S CYCLE TERM DIFFERS FROM mysweep.c's.  `ncy` counts type-I
       AND type-II cycles together (the type-II placement at :175-186 decrements
       both `ncy` and `c2rem`), but a type-I cycle covers 4 rotation classes and
       a type-II cycle covers only 3 -- see this file's header at :106, "(2 nc
       slots, 3 classes, l1+l2=3)", and the endgame at :174-186, which covers
       exactly the three slots j not in {nc1, nc2}.  So the charge is
       4*ncy - c2rem, not 4*ncy.  Charging 4 to every cycle made this engine
       refuse its own committed type-II positive control
       (C2N=1 ./mysweep_c2 4 0 0 0 1 25 0 0 0 0 0 0, CERTIFICATES/
       planted_typeII_pos.log): 20 + 1 + 100 = 121, where the instance is valid
       at 24 type-I x 4 + 1 type-II x 3 + 20 + 1 = 120. */
    {
        if(c2rem < 0 || c2rem > ncy){
            printf("INSTFAIL c2=%d not in [0,ncyc=%d] "
                   "(every type-II cycle is one of the ncyc cycles)\n",
                   c2rem, ncy);
            return 3;
        }
        ...
        if(tot != 120){
            printf("INSTFAIL tot=%d != 120 "
                   "(5n5+4n4+3n3+2n2+n1 + 4*ncyc - c2 + sum(s2) + blen)\n", tot);
            return 3;
        }
    }
    int sat = 0;
