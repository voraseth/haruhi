...
/* ---- WITNESS CAPTURE (G20-FINAL): store the explicit chain that attained
   each table cell.  record() is only ever called at a genuine chain state,
   so segH[0..NB-1]/segL[0..NB-1] is a real class-disjoint hop-chain. ---- */
static int WH[64][4][64], WL[64][4][64], WN[64][4];
...
static void record(int lastShort){
  ...
  if(COST<=CMAX){
    if(NB>G[GI(COST,a,b)]) { G[GI(COST,a,b)]=NB;
      if(COST<64 && NB<=64){ WN[COST][a*2+b]=NB;
        for(int q=0;q<NB;q++){WH[COST][a*2+b][q]=segH[q];WL[COST][a*2+b][q]=segL[q];} } }
...
int main(int argc,char**argv){
  ...
  if(mode[0]=='A'){
    ...
    printf(A_maxblocks==4&&A_maxfours==2&&A_interior_four==0?"RUNCAP4 PASS\n":"RUNCAP4 FAIL\n");
    return 0;
  }
  ...
  printf("WITNESSES  (cost ab blocks : head/len pairs, heads are perm indices 0..719)\n");
  for(int c=0;c<=CMAX;c++)for(int ix=0;ix<4;ix++){
    if(G[GI(c,ix>>1,ix&1)]<0) continue;
    printf("W %d %d %d %d :",c,ix>>1,ix&1,WN[c][ix]);
    for(int q=0;q<WN[c][ix];q++) printf(" %d/%d",WH[c][ix][q],WL[c][ix][q]);
    printf("\n");
  }
  printf("PERMS\n");
  for(int i=0;i<720;i++){printf("P %d",i);for(int k=0;k<6;k++)printf(" %d",P[i][k]);printf("\n");}
  printf("DONE\n");
  return 0;
}
