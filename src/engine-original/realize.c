...
static int dfsA(int end){
    ...
    for(int t=0;t<6;t++){
        ...
        for(int l=5;l>=1;l--){
            ...
            nrem[l]--; npieces++; if(npieces>maxpieces)maxpieces=npieces; rec_push(2,x,l);
            if(dfsA(arc_end(x,l))) return 1;
...
static int dfsQ(int end){
    ...
    if(nodes>CAP){ printf("NODE_CAP_HIT\n"); exit(3); }
    int undo[5], u2[6];
    if(m2_rem==0){                        /* ctrl mode: Q ends at global end */
        int left=0; for(int l=1;l<=5;l++) left+=nrem[l];
        if(left==0) return endgame();
    }
    for(int t=0;t<6;t++){
        ...
        /* (b) close: hop onto K1, place M2, switch to chain A from K2 */
        if(m2_rem){
            for(int v=0;v<M2N;v++){
                int l1=M2L[v][0], l2=M2L[v][1], span=l1+1+l2;
                ...
                m2_rem--; npieces++; if(npieces>maxpieces)maxpieces=npieces; rec_push(3,x,l1*10+l2);
                int k2h=x; for(int j=0;j<l1+1;j++) k2h=rhop[k2h];
                if(dfsA(arc_end(k2h,l2))) return 1;
                seq_n--; npieces--; m2_rem++;
...
int main(int argc, char **argv){
    ...
    if(argc<8){printf("usage: realize n5 n4 n3 n2 n1 ncyc m2spec [cap]\n");return 1;}
    ...
    if(strcmp(argv[7],"0")){
        ...
        while(tok){
            /* E-10: this engine alone takes TWO-DIGIT l1l2 tokens; every other
               engine in the family takes span TOTALS (e.g. "2,3").  A
               single-digit token used to give l2 = '\0'-'0' = -48, so the
               placement loop never ran, m2_rem was consumed anyway, and the
               engine searched a DIFFERENT instance and printed
               "UNSAT ... EXHAUSTED".  Reject it loudly instead. */
            if(strlen(tok)!=2 || tok[0]<'1' || tok[0]>'4'
                              || tok[1]<'1' || tok[1]>'4'){
                printf("M2SPECFAIL token \"%s\": realize takes two-digit l1l2 "
                       "tokens with l1,l2 in 1..4 (e.g. 13,31,22), not span "
                       "totals\n", tok);
                return 3;
            }
            if(M2N>=8){ printf("M2SPECFAIL too many M2 tokens\n"); return 3; }
            M2L[M2N][0]=tok[0]-'0'; M2L[M2N][1]=tok[1]-'0'; M2N++; tok=strtok(NULL,","); }
        m2_rem=1;
    }
    if(argc>8) CAP=atoll(argv[8]);
    if(getenv("CHAINONLY")) chainonly=1;
    /* start: q1 comp at 123456, any remaining length */
