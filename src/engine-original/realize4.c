/* realize4.c -- v4: the corner (15,0,1), e=2 -- the two K2-surviving wirings.
 * MODE zh : pieces = 24 m=1 arcs + A (M2, cost sA) + B = [ncH K(l'')].
 *   Join "zh-close": the zh heavy is a completion path-end exit (zp3=0) whose
 *   landing is the NC start u, at exact d=3: from a chain end, the 6 hop
 *   successors are tried AS u; span [u(nc), rho(u)..rho^l''(u) crits]; the
 *   next chain continues from the K path's end.
 * MODE zp3: pieces = 24 m=1 arcs + B1 (M2, cost s1) + B2 = [K(l3) nc].
 *   Join "b2-close": a normal d=3 hop onto K's head x; span adds the trailing
 *   nc at rho^l3(x); the next chain then starts at one of the 6 exact-d=3
 *   successors H of z = rotinv(rho^l3(x)) (the zp3 exit source), H being a
 *   fresh arc head or B1's closer head (fused variant).
 * Join order linear by R1' (NOTES.md).  ncyc = 0 in both wirings (K2).
 * Derived from realize3.c -- v3: adds the d>=4 join moves for the (15,1,0) and (10,2,0)
...
static int m2sums[6];
static int d4_rem = 0;
static int bmode = 0, b_len = 0, b_rem = 0; /* 1=zh, 2=zp3 */                    /* remaining M2s by s2=l1+l2 (2..4) */
static int found = 0;
...
static int start_chain_at(int x){
    int undo[5], u2[6];
    for(int l=5;l>=1;l--){
        if(!nrem[l]) continue;
        if(!place_arc(x,l,undo)) continue;
        nrem[l]--; npieces++; if(npieces>maxpieces)maxpieces=npieces; rec_push(6,x,l);
        if(dfsQ(arc_end(x,l))) return 1;
        seq_n--; npieces--; nrem[l]++;
        unplace_arc(l,undo);
    }
    if(m2_rem){   /* fused: the zp3 lands on B1's closer head */
        ...
        for(int l1=1;l1<s2;l1++){
            ...
            m2_rem--; m2sums[s2]--; npieces++; if(npieces>maxpieces)maxpieces=npieces; rec_push(7,x,l1*10+l2);
...
static int dfsQ(int end){
    ...
    if(nodes>CAP){ printf("NODE_CAP_HIT\n"); exit(3); }
    int undo[5], u2[6];
    if(m2_rem==0 && d4_rem==0 && b_rem==0){ /* all joins placed: may end at global end */
    ...
    for(int t=0;t<(loose?30:6);t++){
        ...
        /* B-joins for the (15,0,1) wirings */
        if(b_rem && bmode==1){   /* zh-close: x IS the nc start u */
            int u=x, ok=1, q=u;
            if(!((pmask[orbid[u]]>>orbpos[u])&1)){
                int u4[6]; u4[0]=u; q=rhop[u];
                for(int j=1;j<=b_len && ok;j++){
                    if((pmask[orbid[q]]>>orbpos[q])&1 || covget(clsp[q])) ok=0;
                    else { u4[j]=q; q=rhop[q]; }
                }
                if(ok){
                    pmask[orbid[u]]|=1<<orbpos[u];
                    for(int j=1;j<=b_len;j++){ pmask[orbid[u4[j]]]|=1<<orbpos[u4[j]]; covset(clsp[u4[j]]); }
                    b_rem--; npieces++; if(npieces>maxpieces)maxpieces=npieces; rec_push(8,u,b_len);
                    if(dfsQ(arc_end(rhop[u],b_len))) return 1;
                    seq_n--; npieces--; b_rem++;
                    for(int j=1;j<=b_len;j++){ pmask[orbid[u4[j]]]&=~(1<<orbpos[u4[j]]); covclr(clsp[u4[j]]); }
                    pmask[orbid[u]]&=~(1<<orbpos[u]);
                }
            }
        }
        if(b_rem && bmode==2){   /* b2-close: x = K's head; nc trails; then zp3-launch */
            int q=x, ok=1, u5[6];
            for(int j=0;j<b_len && ok;j++){
                if((pmask[orbid[q]]>>orbpos[q])&1 || covget(clsp[q])) ok=0;
                else { u5[j]=q; q=rhop[q]; }
            }
            int u=q; /* the trailing nc */
            if(ok && ((pmask[orbid[u]]>>orbpos[u])&1)) ok=0;
            if(ok){
                for(int j=0;j<b_len;j++){ pmask[orbid[u5[j]]]|=1<<orbpos[u5[j]]; covset(clsp[u5[j]]); }
                pmask[orbid[u]]|=1<<orbpos[u];
                b_rem--; npieces++; if(npieces>maxpieces)maxpieces=npieces; rec_push(9,x,b_len);
                int z=rotinv[u], done=0;
                for(int h=0;h<6 && !done;h++)
                    if(start_chain_at(hop3[z][h])) done=1;
                if(done) return 1;
                seq_n--; npieces--; b_rem++;
                pmask[orbid[u]]&=~(1<<orbpos[u]);
                for(int j=0;j<b_len;j++){ pmask[orbid[u5[j]]]&=~(1<<orbpos[u5[j]]); covclr(clsp[u5[j]]); }
            }
        }
...
int main(int argc, char **argv){
    ...
    if(argc<11){printf("usage: realize4 n5 n4 n3 n2 n1 ncyc s2list bq1 bmode blen [cap]\n");return 1;}
    ...
    int bq1=atoi(argv[8]);
    d4_rem=0;
    bmode=atoi(argv[9]); b_len=atoi(argv[10]); b_rem=(bmode>0);
    if(argc>11) CAP=atoll(argv[11]);
