/* realize2.c -- T-n6-tight stage-2 realizability engine, v2: whole (20,0,0)
...
static int m2sums[6];                    /* remaining M2s by s2=l1+l2 (2..4) */
static int found = 0;
...
static int dfsQ(int end){
    ...
    if(nodes>CAP){ printf("NODE_CAP_HIT\n"); exit(3); }
    int undo[5], u2[6];
    if(m2_rem==0){                        /* all joins placed: may end at global end */
...
int main(int argc, char **argv){
    ...
    if(argc<9){printf("usage: realize2 n5 n4 n3 n2 n1 ncyc s2list bq1 [cap]\n");return 1;}
